    ...
    mounted() {
      ...
      class Snake {
        ...
        getDirection(x, y) {
          ...
          if (x > (conf.FIELD_WIDTH * conf.POINT) - conf.POINT) {
            x = 0;
          }
          if (x < 0) {
            x = (conf.FIELD_WIDTH * conf.POINT) - conf.POINT;
          }
          if (y > (conf.FIELD_HEIGHT * conf.POINT) - conf.POINT) {
            y = 0;
          }
          if (y < 0) {
            y = (conf.FIELD_HEIGHT * conf.POINT) - conf.POINT;
          }
          return [x, y];
        }
      }
